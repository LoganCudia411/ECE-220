...
//partners: cudia2
/* INTRO: 
  This program takes a binary tree structure and utilizes it to store the way a larger rectangle is cut up into smaller rectangle. 
  In this tree, we have module nodes that hold the values of the smaller rectangle and the cutnodes which indicate which way the larger 
  rectangle has been cut. This applicable for VSLI design and optimizing circuit design with the least amount of total area. 

  First we parse through a file and get our data to create a tree from an input file. Our first 2 functions check to see if if we are 
  looking at a leaf node(module) or a internal node(cutline). The tree we make is left skewed and we are able to rotate a leaf node, change a cutline, 
  and swap a entire subtree.  We also have a fucntion to produce a postfix expression for how the modules will make up the larger rectangle
*/
...
int is_leaf_node(node_t* ptr) {
  ...
  if(temp->left==NULL && temp->right==NULL && temp->parent!=NULL && temp->module!=NULL && temp->cutline==UNDEFINED_CUTLINE){
    return 1;
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  ...
  if(ptr->right!=NULL && ptr->left!=NULL){//if node has children, then its internal
    return 1;
  }
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  if(a==NULL || b==NULL){//base case 1: node b not found in any subtree
    return 0;
  }else if(a==b){//a and b are the same root node
    return 1;
  }//recursive case, first check imnmediate left and right node
  else if(a->left==b || a->right==b){
    return 1;
  }else if(is_in_subtree(a->right, b)){//check all right children
    return 1;
...
void rotate(node_t* ptr) {//change height and width
  // TODO: 
...
void recut(node_t* ptr) {//change the cutline with vertical if the current cutile is horizontal and vise versa
...
void swap_module(node_t* a, node_t* b) {//module of node a equals module of node b
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t *tempA = a->parent; //save a parent
  node_t *tempB = b->parent; //save b parent
  ...
  if(tempA->left == a) //if a is on the left
  {
    tempA->left =b; 
    b->parent = tempA; 
  }
  if(tempA->right == a) //if a is on the right
  {
    tempA->right = b; 
    b->parent = tempA;
  }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if(ptr == NULL) return;
  postfix_traversal(ptr->left, nth, expression);//check left
  postfix_traversal(ptr->right, nth, expression);//check right
...
int get_total_resource(node_t* ptr)//sum the total resources of the left, then of the right and return the sum of both
{
  int x=0, y=0;
  if(ptr==NULL){//base case 1 (no more nodes)
    return 0;
  }else if(is_leaf_node(ptr)){//base case 2 (we encounter a leaf node, return resource)
    return ptr->module->resource;
  }
  if(ptr->left!=NULL){
    x=get_total_resource(ptr->left);
  }
  if(ptr->right!=NULL){
    y=get_total_resource(ptr->right);
  }
  return x+y;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  assert(n >= 0 && n < num_modules);
   ...
   if (n == num_modules - 1) {
    ...
    //reate a new internal node 
    node_t* internal = (node_t*)malloc(sizeof(node_t));
    internal->module = NULL;
    internal->parent = par;
    internal->cutline = V;
    // create a leaf node to attach to right of internal node
    ...
    node_t* left = init_slicing_tree(internal, n+1);
